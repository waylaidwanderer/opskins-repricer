...
class OPAccount {
    ...
    async start() {
        const balance = await this.opskins.getBalanceAsync();
        console.log(`[${this.apiKey}] Current balance: $${(balance / 100).toFixed(2)}`);
    ...
    async repriceItems(page = 1) {
        ...
        chunks.forEach(async (chunk) => {
            ...
            await this.editPrices(priceEdits);
        });
        if (page === totalPages) return Promise.resolve();
        return this.repriceItems(page + 1);
    }

    async editPrices(priceEdits) {
        const tag = '[Edit Prices]';
        try {
            await this.opskins.editPricesAsync(priceEdits);
        } catch (err) {
            const errString = err.toString();
            console.log(`${tag} Error editing prices for some items on OPSkins: ${err}`);
            if (errString.includes('Another EditPriceMulti request is currently processing.') || errString.includes('Origin Time-out')) {
                console.log(`${tag} Retrying in one minute...`);
                await new Promise(resolve => setTimeout(resolve, 60000));
                return this.editPrices(priceEdits);
            }
        }
        return Promise.resolve();
    }
